...
export function PageHero({ title, subtitle, path }: PageHeroProps) {
  return (
    <section className="relative border-b border-red-500/20 bg-black overflow-hidden" aria-labelledby="page-title">
      ...
      <div
        className="container mx-auto max-w-7xl relative z-10 text-center px-4 sm:px-6 lg:px-8 pt-[5.5rem] md:pt-28 pb-10 md:pb-12"
      >
        {path && <Breadcrumbs path={path} currentTitle={title} />}
        <h1 id="page-title" className="text-4xl md:text-5xl lg:text-6xl font-bold text-white font-orbitron mt-2 mb-3 md:mb-4 fade-in">
          {title}
        </h1>
        {subtitle && (
          <p
